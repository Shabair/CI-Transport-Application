...
class History extends MY_Controller {
	...
	function index(){
		...
		$data['title'] = 'Company History : DMS';
		$data['view']  = 'company/history';
	...
	function datatable_json(){
		...
		{  
			...
			$data[]= array(
				$count++,
				get_from('users','first_name',['id'=>$r['user_id']]),
	...
	function history_search(){
		...
		$SQL ='SELECT h.* FROM history as h ';
			...
			$wh[]=" h.company_id = '".$this->get_company_id()."'";
	...
	function LoadJson($SQL,$EXTRA_WHERE='')
	{
		if(!empty($_GET['search']['value']) || $_GET['order'][0]['column'] != 6){
			$SQL.='inner join users as u on h.user_id = u.id inner join drivers as d on h.action_id = d.id';
		}
