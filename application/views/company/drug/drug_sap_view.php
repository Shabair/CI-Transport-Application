...
<?php $quarterForJS = str_replace("-","_",get_data('quarter')); ?>
<?php 
        ...
        <?php endif; ?>
            <div class="panel-body">
                ...
                <div class=" form">
                        ...
                        ?>
                        <div class="row" id="driver_id_div_<?php echo get_data('quarter')?>">
...
var target = document.getElementById('driver_id_div_<?php echo get_data('quarter')?>');
  ...
  mutations.forEach(function(mutation) {
    ...
    if( removedNodes.length > 0){
        ...
        if(check_r_div  == 'driver_div_'){
            ...
            if($('#sap_row_'+remove_id+"_<?php echo get_data('quarter')?>").length == 1){
                $('#sap_row_'+remove_id+"_<?php echo get_data('quarter')?>").remove();
            }
        }
    }
...
/**/
    ...
    function dtest_cmplt_func_<?php echo $quarterForJS?>(value,num){
        if(value){
            $("#dtest_cmplt_div_"+num+"_<?php echo get_data('quarter')?>").fadeIn(200);
            //$('#sap_row_'+num).fadeIn(200);
            
        }else{
            $("#dtest_cmplt_div_"+num+"_<?php echo get_data('quarter')?>").fadeOut(200);
            $('#sap_row_'+num+"_<?php echo get_data('quarter')?>").remove();
    ...
    function sap_app_func_<?php echo $quarterForJS?>(value,num){
        if(value){
            $("#sap_app_div_"+num+"_<?php echo get_data('quarter')?>").fadeIn(200);
        }else{
            $("#sap_app_div_"+num+"_<?php echo get_data('quarter')?>").fadeOut(200);
        }
    }

    function sap_letter_func_<?php echo $quarterForJS?>(value,num){
        if(value){
    ...
    function rtd_switch_func_<?php echo $quarterForJS?>(value,num){
        if(value){
            $("#rtd_cmplt_div_"+num+"_<?php echo get_data('quarter')?>").fadeIn(200);
        }else{
            $('#follow_up_div_'+num+"_<?php echo get_data('quarter')?>").empty();
            $("#rtd_cmplt_div_"+num+"_<?php echo get_data('quarter')?>").fadeOut(200);
    ...
    function sap_finst_func_<?php echo $quarterForJS?>(value,num){
        if(value){
            $("#sap_fi_nite_div_"+num+"_<?php echo get_data('quarter')?>").fadeIn(200);
        }else{
            $("#sap_fi_nite_div_"+num+"_<?php echo get_data('quarter')?>").fadeOut(200);
        }
    }

    function fup_dtcmplt_func_<?php echo $quarterForJS?>(value,num,count){
        //console.log('#fup_dcom_div_'+value+'_'+count);
        if(value){
            $('#fup_dcom_div_'+num+'_'+count+"_<?php echo get_data('quarter')?>").fadeIn(200);
        }else{

            $('#fup_dcom_div_'+num+'_'+count+"_<?php echo get_data('quarter')?>").fadeOut(200);
            ...
            if (get_data('driver_id') != '') {
                ...
                if($value != null){
                    ...
                    for($i =0; $i< count($value);$i++){
                        ...
                        childDiv = $("#driver_id_div_<?php echo get_data('quarter')?>").children("#driver_div_"+<?php echo $value[$i]?>).length;
                        if(childDiv != 1){
                            
                            driver_name = "<?php echo getDriverName($value[$i])?>";
                            $("#driver_id_div_<?php echo get_data('quarter')?>").append('<div id="driver_div_'+<?php echo $value[$i]?>+'" style="<?php echo ($this->router->fetch_class() == 'drug')?' border-bottom: 1px solid #FF5C5C':''; ?>;margin-bottom:30px;"><div class="col-md-2"><div class="form-group "> <label for="location" class="control-label ">Driver Name</label><h4>'+driver_name+'</h4></div></div><div class="col-md-2"><div class="form-group "> <label for="is_dna_'+<?php echo $value[$i]?>+'" class="control-label">Drug/Alcohol</label><div class="form-control"> <?php echo (empty(get_data( 'is_dna_'.$value[$i])) )? 'Empty': ''; ?> <?php echo (get_data( 'is_dna_'.$value[$i])=='drug' )? 'Drug': ''; ?> <?php echo (get_data( 'is_dna_'.$value[$i])=='alcohol' )? 'Alcohol': ''; ?> <?php echo (get_data( 'is_dna_'.$value[$i])=='both' )? 'Drug & Alcohol': ''; ?></div></div></div><div id="drug_alcohol_selected_'+<?php echo $value[$i]?>+'_<?php echo get_data('quarter')?>"></div><div class="clearfix"></div></div>');
                        ...
                        ?>
                            ...
                            dtest_cmplt_func_<?php echo $quarterForJS;?>(true,<?php echo $value[$i]?>);
                        ...
                        ?>
                                sap_finst_func_<?php echo $quarterForJS?>(true,<?php echo $value[$i]?>);
                        <?php
                            endif;
                        ...
                        <?php
                                    ...
                                    ?>
                                        ...
                                        ?>
                                            ...
                                            ?>
                                                    fup_dtcmplt_func_<?php echo $quarterForJS?>(true,<?php echo $value[$i]?>,<?php echo $fupi?>);
                                            <?php
                                                endif;
                                        // fup_dtcmplt_func
                                        endfor;
